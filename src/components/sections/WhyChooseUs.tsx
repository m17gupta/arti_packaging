...
function FeatureItem({ item, index }: { item: WhyChooseUsItem; index: number }) {
  return (
    <motion.div
      ...
      className="p-6 border border-[#2E2E2E] bg-[#1A1A1A] group transition-all duration-300 hover:bg-[#1F1F1F]"
    >
      <div className="text-2xl text-[#C9A84C] mb-4 font-serif">{item.icon}</div>
      <h3 className="text-white font-medium text-base mb-2 group-hover:text-[#C9A84C] transition-colors duration-300">
        {item.title}
      </h3>
      <p className="text-[#8A8A8A] text-sm leading-relaxed">{item.description}</p>
    </motion.div>
  )
}
    ...
    <section className="py-28 px-6 bg-[#111111]">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="mt-16 p-8 border border-[#C9A84C]/20 bg-[#1A1A1A] text-center"
        >
          <p className="font-serif text-2xl md:text-3xl font-light text-white leading-relaxed">
